...
class BurpExtender(IBurpExtender, IHttpListener, ITab, IContextMenuFactory):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        print("[LinkedInScapper] Extension loaded successfully!")
    ...
    def _add_profile(self, profile):
        ...
        self._table_model.addRow(
            [profile.get("name", "N/A"), profile.get("position", "N/A"), profile.get("location", "N/A"), profile.get("profile_url", "N/A"), profile.get("badge", "N/A"), profile.get("timestamp", "N/A")]
        )
        return True
    ...
    def _replay_proxy_history(self):
        ...
        for item in proxy_history:
            ...
            # Apply the same LinkedIn API detection logic
...
Author: @two06
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        ...
        is_linkedin_api = False
        if "/voyager/api/graphql" in request_str:
            self._debug_log("[LinkedInScapper] DEBUG: GraphQL API detected")
            is_linkedin_api = True
        elif "/voyager/api/search" in request_str:
            self._debug_log("[LinkedInScapper] DEBUG: Search API detected")
            is_linkedin_api = True
        elif "linkedin.com" in request_str and "api" in request_str:
            self._debug_log("[LinkedInScapper] DEBUG: General LinkedIn API detected")
            is_linkedin_api = True
    ...
    def _sanitize_text(text):
        """Normalize Unicode to closest ASCII representation, stripping what can't be converted"""
        if not text:
            return text
        try:
            # Handle Jython unicode and Python 2 str
            if isinstance(text, unicode):
                import unicodedata

                # Normalize composed characters (e.g. accented letters) to decomposed form
    ...
    def _extract_profile_data(self, item):
        ...
        # Sanitize all text fields to ASCII
        for key in profile_data:
            if isinstance(profile_data[key], (str, unicode)):
                profile_data[key] = self._sanitize_text(profile_data[key])
